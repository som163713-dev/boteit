...
if os.path.isdir(CUSTOMER_DIR):
    app.mount("/customer-assets", StaticFiles(directory=CUSTOMER_DIR), name="customer")
...
async def customer_panel():
    panel = os.path.join(CUSTOMER_DIR, "index.html")
    if os.path.isfile(panel):
        return FileResponse(panel)
    return JSONResponse({"error": "customer panel not found"}, status_code=404)
