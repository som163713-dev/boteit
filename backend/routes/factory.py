...
from backend.agent_access import is_agent_enabled, catalog_with_status
...
def _require_agent(customer, agent_key: str):
    if not is_agent_enabled(customer, agent_key):
        raise HTTPException(
            status_code=403,
            detail=f"دسترسی به ایجنت «{agent_key}» برای پلن/حساب شما فعال نیست. با پشتیبانی تماس بگیرید.",
        )


@router.get("/access")
async def factory_access(api_key: str = Query(...), db: Session = Depends(get_db)):
    """لیست ایجنت‌ها + وضعیت فعال/غیرفعال برای همین مشتری — پنل مشتری از این استفاده می‌کنه."""
    customer = get_customer(api_key, db)
    return {"customer": {"name": customer.name, "business": customer.business, "plan": customer.plan},
...
async def generate_content(req: ContentRequest, db: Session = Depends(get_db)):
    ...
    _require_agent(customer, "content")
...
async def generate_campaign(req: CampaignRequest, db: Session = Depends(get_db)):
    customer = get_customer(req.api_key, db)
    _require_agent(customer, "campaign")
...
async def generate_report(api_key: str = Query(...), days: int = Query(7, ge=1, le=90),
    ...
    customer = get_customer(api_key, db)
    _require_agent(customer, "report")
