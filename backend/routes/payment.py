...
async def payment_request(
    ...
    if not ZARINPAL_MERCHANT:
        raise HTTPException(status_code=503, detail="درگاه پرداخت پیکربندی نشده است")
    ...
    try:
        ...
        if resp.get("data", {}).get("code") == 100:
            ...
            raise HTTPException(status_code=400, detail="خطا در ایجاد پرداخت")
...
async def payment_verify(
    ...
    payment = db.query(Payment).filter(Payment.ref_id == Authority).first()
    if not payment:
    ...
    try:
        ...
        if resp.get("data", {}).get("code") in [100, 101]:
