...
async def dashboard(admin=Depends(get_admin), db: Session = Depends(get_db)):
    try:
        now = datetime.now()
        ...
        today_messages = db.query(Message).filter(Message.created_at >= today).count()
        ...
        month_revenue = db.query(func.sum(Payment.amount)).filter(
            Payment.status == "success",
            Payment.created_at >= now.replace(day=1, hour=0, minute=0, second=0)
        ).scalar() or 0
        ...
        ).outerjoin(Message).group_by(Customer.id)\
