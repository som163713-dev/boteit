...
async function loadCustomers() {
    try {
        ...
        tbody.innerHTML = customers.map(c => {
            const miniappLink = `${window.location.origin}/?key=${c.api_key}`;
            return `
            <tr>
                <td>${c.name}</td>
                <td>${c.business}</td>
                ...
                <td>
                    <span class="api-key" 
                          ...
                          title="کلیک برای کپی کامل API Key">
                        ${c.api_key.substring(0, 16)}...
                    </span>
                </td>
                <td>
                    <button class="action-btn link-btn" 
                            onclick="openMiniapp('${c.api_key}')"
                            title="باز کردن صفحه مینی‌اپ این مشتری">
                        🔗 باز کردن
                    </button>
                    <button class="action-btn copy-btn" 
                            onclick="copyMiniappLink('${c.api_key}')"
                            title="کپی لینک مینی‌اپ">
                        📋 کپی لینک
                    </button>
                </td>
                <td>
...
async function addCustomer() {
    ...
    try {
        ...
        if (res.ok) {
            const link = `${window.location.origin}/?key=${result.api_key}`;
            // کپی خودکار لینک
            try { navigator.clipboard.writeText(link); } catch(e) {}
            alert(
                `✅ مشتری اضافه شد!\n\n` +
                `API Key:\n${result.api_key}\n\n` +
                `لینک مینی‌اپ اختصاصی (کپی شد):\n${link}\n\n` +
                `این لینک را به مشتری بدهید یا از ستون «لینک مینی‌اپ» در جدول باز کنید.`
            );
            hideAddCustomer();
            loadCustomers();
            loadStats();
...
// ══ کپی API Key و لینک مینی‌اپ ══════════════════
function copyKey(key) {
    navigator.clipboard.writeText(key).then(() => {
        alert('API Key کامل کپی شد! ✅');
    }).catch(() => {
        prompt('API Key را کپی کنید:', key);
    });
}

function getMiniappLink(apiKey) {
    return `${window.location.origin}/?key=${apiKey}`;
}

function openMiniapp(apiKey) {
    const link = getMiniappLink(apiKey);
    window.open(link, '_blank');
}

function copyMiniappLink(apiKey) {
    const link = getMiniappLink(apiKey);
    navigator.clipboard.writeText(link).then(() => {
        alert('لینک مینی‌اپ کپی شد! ✅\n\n' + link);
    }).catch(() => {
        prompt('لینک مینی‌اپ را کپی کنید:', link);
    });
}
